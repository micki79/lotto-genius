#!/usr/bin/env python3
"""
🧠 LottoGenius - Glücksspirale Lern-System mit Selbstlernen

Lernt aus jeder Ziehung:
1. Zählt übereinstimmende Endziffern (Gewinnklassen 1-7)
2. Aktualisiert Strategie-Gewichte
3. Trackt Performance über Zeit

Besonderheit: Glücksspirale hat 7 Gewinnklassen!
...
GEWINNKLASSEN = {
    ...
    2: {'endziffern': 6, 'name': '6 Endziffern', 'weight_boost': 4.5},
    3: {'endziffern': 5, 'name': '5 Endziffern', 'weight_boost': 3.5},
...
def count_matching_end_digits(predicted, actual):
    """Zählt übereinstimmende Endziffern von rechts nach links."""
...
def determine_gewinnklasse(end_matches):
    """Bestimmt die Gewinnklasse basierend auf Endziffern."""
...
def learn_from_results():
    print("=" * 60)
    print("🧠 LottoGenius - Glücksspirale Lern-System")
    print("=" * 60)
    ...
    learning = load_json('gluecksspirale_learning.json', {'entries': [], 'stats': {}})
    ...
    print(f"📅 Letzte Ziehung (Samstag): {draw_date}")
    ...
    for pred in unverified:
        pred_num = str(pred.get('number', '0000000')).zfill(NUM_DIGITS)
    ...
    save_json('gluecksspirale_learning.json', learning)
